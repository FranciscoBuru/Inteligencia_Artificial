...
;(setf lst (list 0 0 0 0 0 0 7(list 1 2 3 4 5 6 7 0 8)))
...
(defun leeInicio()
	(defparameter *s* (open "entrada.txt"))
	(setf l1 (read *s*))
	(setf lst (list 0 0 0 0 0 0 (position 0 l1) l1)))	

(defun clean()
	(setf *open* nil)
	(setf *closed* nil)
	(setf *nodeCounter* 0)
	(setf final (list 1 2 3 4 5 6 7 8 0))
	(leeInicio))
...
(defun main ()
	(leeInicio)
	(if (estrella lst) 
	(with-open-file (*standard-output* "somefile.dat" :direction :output :if-exists :supersede)(write (format nil "~{~a~% ~}" *closed*)))
	(with-open-file (*standard-output* "somefile.dat" :direction :output :if-exists :supersede)(write 'Todo_Mal)))
	(with-open-file (*standard-output* "generados.dat" :direction :output :if-exists :supersede)(write *nodeCounter*)))
(main)
